...
TextTagComponent::TextTagComponent(const std::string &tagName, const util::TSharedHandle<AnchorPoint> &startAnchor, const util::TSharedHandle<AnchorPoint> &endAnchor) : m_tagName {tagName}, m_startAnchor {startAnchor}, m_endAnchor {endAnchor} {}
bool TextTagComponent::operator==(const TextTagComponent &other) const { return IsValid() && other.IsValid() && *m_startAnchor == *other.GetStartAnchorPoint() && *m_endAnchor == *other.GetEndAnchorPoint() && m_tagName == other.m_tagName; }
...
bool TextTagComponent::IsValid() const { return m_startAnchor.IsValid() && m_endAnchor.IsValid() && m_endAnchor->GetTextCharOffset() > m_startAnchor->GetTextCharOffset(); }
TextLength TextTagComponent::GetLength() const { return m_endAnchor->GetTextCharOffset() - m_startAnchor->GetTextCharOffset() + 1; }
const std::string &TextTagComponent::GetTagName() const { return m_tagName; }

//////////////

TextOpeningTagComponent::TextOpeningTagComponent(const std::string &tagName, const std::string &label, const std::vector<std::string> &attributes, const util::TSharedHandle<AnchorPoint> &startAnchor, const util::TSharedHandle<AnchorPoint> &endAnchor)
...
bool TextOpeningTagComponent::operator==(const TextTagComponent &other) const { return typeid(other) == typeid(TextOpeningTagComponent) && operator==(static_cast<const TextOpeningTagComponent &>(other)); }
bool TextOpeningTagComponent::operator==(const TextOpeningTagComponent &other) const { return TextTagComponent::operator==(static_cast<const TextTagComponent &>(other)) && m_label == other.m_label && m_attributes == other.m_attributes; }

//////////////

TextTag::TextTag(const FormattedText &text, const util::TSharedHandle<TextTagComponent> &openingTag, const util::TSharedHandle<TextTagComponent> &closingTag) : m_wpText {text.shared_from_this()}, m_openingTag {openingTag}, m_closingTag {closingTag} {}
bool TextTag::IsValid() const { return m_wpText.expired() == false && m_openingTag.IsValid() && m_openingTag->IsValid() && (m_openingTag.IsExpired() || m_openingTag->IsValid()); }
bool TextTag::IsClosed() const { return m_closingTag.IsValid() && m_closingTag->IsValid(); }
